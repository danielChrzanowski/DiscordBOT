...
const slashCommandBuilder = new SlashCommandBuilder()
  ...
  .addStringOption((option) =>
    option.setName('nick').setDescription('Player nickname to get info about').setRequired(false),
  );
  ...
  async executeSlash(client: Client, interaction: ChatInputCommandInteraction) {
    ...
    try {
      if (nick) {
        ...
        const { result } = await fetch('https://ps2.fisu.pw/api/population/?world=10').then((response: any) =>
          response.json(),
        );
            ...
            datasets: [
              {
                ...
                backgroundColor: ['rgba(125, 11, 165, 1)', 'rgba(11, 90, 165, 1)', 'rgba(165, 11, 11, 1)'],
                borderColor: ['rgba(125, 11, 165, 1)', 'rgba(11, 90, 165, 1)', 'rgba(165, 11, 11, 1)'],
              },
            ],
          },
          options: {
